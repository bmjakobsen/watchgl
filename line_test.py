...
class WatchGraphics():
    ...
    def draw_line(self, color:int, width:int, x0:int, y0:int, x1:int, y1:int) -> None:
        ...
        while True:
            ...
            x_offset3:int = x_offset+x_offset2
            y_offset3:int = y_offset+y_offset2
            rwidth:int = width+width_change
            rheight:int = width+height_change

            if rwidth <= 0 or rheight <= 0:
                remaining_repeats = 0
            elif y_offset3 == 0 and rheight == buffer[pos-4+3] and remaining_repeats > 0:
                if x_offset3 < 0:
                    buffer[pos-4+0] += x_offset3
                    buffer[pos-4+2] -= x_offset3
                else:
                    buffer[pos-4+2] += x_offset3
                remaining_repeats -= 1
            elif x_offset3 == 0 and rwidth == buffer[pos-4+2] and remaining_repeats > 0:
                if y_offset3 < 0:
                    buffer[pos-4+1] += y_offset3
                    buffer[pos-4+3] -= y_offset3
                else:
                    buffer[pos-4+3] += y_offset3
                remaining_repeats -= 1
            else:
                buffer[pos] = x_offset3
                buffer[pos+1] = y_offset3
                buffer[pos+2] = rwidth
